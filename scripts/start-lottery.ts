...
import lotteryABI from "../abi/DexLottery.json";
...
const main = async () => {
  ...
  if (networkName === "testnet" || networkName === "mainnet") {
    ...
    // Check if the Dex Lottery / Chainlink Oracle smart contract addresses are set.
    if (
    ...
    try {
      ...
      // );
      const ticketPriceinSpc = '10'
      // Create, sign and broadcast transaction.
      const tx = await contract.startLottery(
        getEndTime(),
        parseUnits(ticketPriceinSpc, 9),
